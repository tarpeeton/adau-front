...
const ServiceCase: FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const CaseDataAll = await client.fetch(`*[_type == "case" && isSuccessfulProject == true]`)
      ...
      <div className='flex flex-col'>
        <div className='flex flex-col 2xl:flex-row 2xl:flex-wrap 2xl:gap-[20px]'>
          ...
          {caseData.map((item, index) => (
            <div key={index} className='mt-[30px] 2xl:mt-[30px] 2xl:w-[48%]'>
              <div className='relative '>
                ...
                <div className='absolute bottom-[20px] left-1/2 transform -translate-x-1/2 z-[99] flex items-center gap-[15px] justify-center'>
                  {item.slider.map((_, i) => (
                    <div
                      key={i}
                      className={`w-[25px] h-[3px] ${i === activeIndices[index] ? 'bg-white' : 'bg-inherit backdrop-blur-[15px]'}`}
                    ></div>
                  ))}
                </div>
